...
from condizioni import valida_condizioni_caricatore

class CaricatoreControllo(tk.Frame):
    def __init__(self, parent, controller, invia_comando_fn, leggi_stato_fn):
        ...
        # --- Sezione Posizione di manutenzione ---
        self.frame_maintenance = tk.LabelFrame(self.frame_pulsanti, text="Posizione di manutenzione:", font=("Arial", 12, "bold"))
        self.frame_maintenance.pack(fill="x", padx=5, pady=5)

        # --- Pulsanti Posizione di manutenzione affiancati ---
        ...
        # --- Etichetta stato Posizione di manutenzione ---
    ...
    def aggiorna_comando(self, cmd, stato_macchina):
        ...
        if cmd in comando_to_flag:
            ...
            condizioni, ok, _ = valida_condizioni_caricatore(
    ...
    def invia_comando_enable_drive(self, valore):
        comando_str = f"write Caricatore CMD_EnableDrive {valore}"
        print(f"[DEBUG] Invio comando: {comando_str}")

        if self.invia_comando_fn:
            self.invia_comando_fn(comando_str)
            self.controller.dati_macchine["Caricatore"]["Stato_EnableDrive"] = (valore == -1)
        else:
            print("[ERRORE] invia_comando_fn non definita!")

    def invia_comando_enable_auto(self, valore):
        comando_str = f"write Caricatore CMD_Automatico {valore}"
        print(f"[DEBUG] Invio comando: {comando_str}")

        if self.invia_comando_fn:
            self.invia_comando_fn(comando_str)
            self.controller.dati_macchine["Caricatore"]["Stato_Automatico"] = (valore == -1)
        else:
            print("[ERRORE] invia_comando_fn non definita!")

    def invia_comando_enable_maintenance_position(self, valore):
        comando_str = f"write Caricatore CMD_MaintenancePosition {valore}"
        print(f"[DEBUG] Invio comando: {comando_str}")

        if self.invia_comando_fn:
            self.invia_comando_fn(comando_str)
            self.controller.dati_macchine["Caricatore"]["Stato_MaintenancePosition"] = (valore == -1)
        else:
            print("[ERRORE] invia_comando_fn non definita!")

    def invia_comando_home(self):
        stato_home = self.controller.dati_macchine.get("Caricatore", {}).get("Home_OK", False)

        if stato_home:
            risposta = messagebox.askyesno("Conferma HOME", "Il caricatore è già azzerato.\nSicuro di voler continuare?")
            if not risposta:
                print("[DEBUG] Home annullato dall'utente.")
                return

        comando_str = f"write Caricatore CMD_Home -1"
        print(f"[DEBUG] Invio comando: {comando_str}")
    ...
    def invia_comando_reset(self):
        ...
        comando_str = f"write Caricatore CMD_Reset -1"
        print(f"[DEBUG] Invio comando: {comando_str}")
    ...
    def aggiorna_help(self, cmd):
        ...
        """
        condizioni, _, tooltip = valida_condizioni_caricatore(
            self.controller.dati_macchine,
            "Caricatore",
            cmd
        )
