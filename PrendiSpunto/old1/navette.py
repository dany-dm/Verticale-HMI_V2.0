...
class PaginaNavette(tk.Frame):
    ...
    def aggiorna_gui(self, dati_navetta, indice_navetta):
        navetta_key = f"Navetta_{indice_navetta+1}"
        stato = dati_navetta.get(navetta_key)
    ...
    def aggiorna_barra_corsa(self):
        ...
        stato = self.controller.dati_navetta.get(navetta_key, {})
    ...
    def refresh_gui_periodico(self):
        if self.controller.current_page_name == "PaginaNavette":
            self.aggiorna_gui(self.controller.dati_navetta, self.indice_navetta)
