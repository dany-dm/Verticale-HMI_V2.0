# carrello_controllo.py

import tkinter as tk
from tkinter import messagebox
from condizioni import valida_condizioni_carrello

class CarrelloControllo(tk.Frame):
    ...
    def aggiorna_comando(self, cmd, stato_macchina):
        ...
        if cmd in comando_to_flag:
            ...
            condizioni, ok, _ = valida_condizioni_carrello(
    ...
    def invia_comando_enable_drive(self, valore):
        comando_str = f"write Carrello CMD_EnableDrive {valore}"
        print(f"[DEBUG] Invio comando: {comando_str}")

        if self.invia_comando_fn:
            self.invia_comando_fn(comando_str)
            self.controller.dati_macchine["Carrello"]["Stato_EnableDrive"] = (valore == -1)
        else:
            print("[ERRORE] invia_comando_fn non definita!")

    def invia_comando_enable_auto(self, valore):
        comando_str = f"write Carrello CMD_Automatico {valore}"
        print(f"[DEBUG] Invio comando: {comando_str}")

        if self.invia_comando_fn:
            self.invia_comando_fn(comando_str)
            self.controller.dati_macchine["Carrello"]["Stato_Automatico"] = (valore == -1)
        else:
            print("[ERRORE] invia_comando_fn non definita!")

    def invia_comando_enable_maintenance_position(self, valore):
        comando_str = f"write Carrello CMD_MaintenancePosition {valore}"
        print(f"[DEBUG] Invio comando: {comando_str}")

        if self.invia_comando_fn:
            self.invia_comando_fn(comando_str)
            self.controller.dati_macchine["Carrello"]["Stato_MaintenancePosition"] = (valore == -1)
        else:
            print("[ERRORE] invia_comando_fn non definita!")

    def invia_comando_home(self):
        stato_home = self.controller.dati_macchine.get("Carrello", {}).get("Home_OK", False)

        if stato_home:
            risposta = messagebox.askyesno("Conferma HOME", "Il carrello è già azzerato.\nSicuro di voler continuare?")
            if not risposta:
                print("[DEBUG] Home annullato dall'utente.")
                return

        comando_str = f"write Carrello CMD_Home -1"
        print(f"[DEBUG] Invio comando: {comando_str}")
    ...
    def invia_comando_reset(self):
        ...
        comando_str = f"write Carrello CMD_Reset -1"
        print(f"[DEBUG] Invio comando: {comando_str}")
    ...
    def aggiorna_help(self, cmd):
        ...
        """
        condizioni, _, tooltip = valida_condizioni_carrello(
            self.controller.dati_macchine,
            "Carrello",
            cmd
        )
