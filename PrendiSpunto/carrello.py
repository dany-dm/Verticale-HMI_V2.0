...
from carrello_controllo import CarrelloControllo
...
class PaginaCarrello(tk.Frame):
    def __init__(self, parent, controller, invia_comando_fn, leggi_stato_fn):
        ...
        label_macchina = tk.Label(frame_comunicazione, text="Carrello", font=("Arial", 20, "bold"))
        ...
        for col_idx, gruppo in enumerate(colonne):
            ...
            for descrizione, chiave in gruppo:
                ...
                if col_idx == 0 and chiave == "Y_Homed":
                    ...
                    self.stato_labels["Y_Encoder"] = val
        ...
        self.controllo = CarrelloControllo(
            frame_comandi,
            self.controller,
        ...
        # --- Frame barra Y ---
        frame_barra_y = tk.Frame(contenitore_colonne)
        frame_barra_y.grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
        # TRUCCO: faccio crescere la riga 0 di frame_barra_y
        frame_barra_y.grid_rowconfigure(0, weight=1)
        frame_barra_y.grid_columnconfigure(0, weight=1)
        # Canvas barra Y — usiamo grid, NON pack!
        self.barra_Y = tk.Canvas(frame_barra_y, width=100, bg="white")
        self.barra_Y.grid(row=0, column=0, sticky="nsew")
        self.barra_Y.bind("<Configure>", lambda e: self.aggiorna_barra_y())
    ...
    def aggiorna_gui(self, dati_macchine):
        ...
        self.aggiorna_barra_y()

    def aggiorna_barra_y(self):

        try:
            corsa_max_y = float(self.controller.configurazione["carrello"]["corsa_max_y"])
        except:
            corsa_max_y = 5000

        stato = self.controller.dati_macchine.get("Carrello", {})
        ...
        if self.last_y_encoder == y_encoder:
            return
        ...
        offset = 100
        offset_macchina = 1100
        total_height_mm = corsa_max_y + offset + offset + offset_macchina
        ...
        rect_y = y_end - ((y_encoder + offset_macchina) * scala)
        ...
        indicatori = [
            #(1800 - offset, "home"),
            (1540 - offset, "caricatore")
            #(corsa_max_y - offset, f"{int(corsa_max_y)} mm")
        ]
    ...
    def refresh_gui_periodico(self):
        if self.controller.current_page_name == "PaginaCarrello":
